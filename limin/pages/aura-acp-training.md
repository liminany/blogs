...
- 待办事项梳理，backlog grooming：团队与PO讨论故事和其完成方法，同时做出工作中的一些初始估算。一般是在当前迭代快经结束时，或者是在迭代间的缓冲期闪，主要目是为了下一次迭代做准备
- 冲突的五个阶段：发现问题；出现分歧；为分歧产生竞争；斗争；世界大战
- 用户故事创建的方法：访谈、调查问卷、观察、和故事写作培训
- PO、客户、团队都可以创建待办事项，但是PO对待办事件负责
- PO应该有的素质：承担 负责任 授权 协作 知识渊博
...
- 水晶开发方法一个首要原则就是渗透沟通，参透沟通来源于水晶开发方法。水晶开发方法的三个基本原则是：小团队、频繁交付、参透沟通
